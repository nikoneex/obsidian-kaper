...
| 2 | `document.createElement` vs `createEl` | Fixed. Swapped `view.dom.ownerDocument.createElement('div')` in [editor-extension.tsx:81](../src/editor-extension.tsx#L81) for Obsidian's global `createDiv({ cls: 'kaper-block' })`, which returns a detached element and satisfies both CodeMirror's `toDOM()` contract and the scanner. The earlier "no change" resolution was based on a wrong assumption that all `createDiv` helpers append — the Node-prototype method does, but the global helper doesn't ([obsidian.d.ts:193](../node_modules/obsidian/obsidian.d.ts#L193)). Remaining `createElement` hits in the built `main.js` come from bundled React internals (5 of them) — not our source. | `grep -c createElement src/editor-extension.tsx` → `0`; `npm run lint` clean; `npm test` (92/92 green). |
